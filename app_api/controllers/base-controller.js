class BaseController {
    ...
    select(req, res) {  
        ...
        let {skip=0, limit=process.env.SELECT_LIMIT, fields, sort, ...filter} = req.query;
        ...
        this.Entity.find(filter).countDocuments( (err, total)=> {
            ...
            this.Entity.find(filter).select(fields).skip(+skip).limit(+limit).sort(sort).exec( (err, data) => {
